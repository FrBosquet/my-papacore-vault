import { getLeaf } from '../../utils/files'
import { createNewReadLater } from '../../utils/templater'
import { Button } from '../shared/button'
import { Dialog, useDialog } from '../shared/dialog'

export const AddReadLaterModal = () => {
  const { ref: dialogRef, close } = useDialog()

  const handleSubmit = async (e: Event) => {
    e.preventDefault()
    const formData = new FormData(e.target as HTMLFormElement)
    const title = formData.get('title') as string
    const url = formData.get('url') as string
    const why = formData.get('why') as string

    const file = await createNewReadLater(title, url, why)

    if (file) {
      getLeaf(true).openFile(file)
    }

    close()
  }

  return (
    <Dialog
      dialogRef={dialogRef}
      title="Add to reading list"
      triggerProps={{
        icon: 'plus',
        size: 'icon-xs',
      }}
    >
      <form className="flex flex-col gap-6 items-end" onSubmit={handleSubmit}>
        <input
          type="text"
          name="title"
          placeholder="Title"
          className="w-full"
        />
        <input type="text" name="url" placeholder="URL" className="w-full" />
        <input type="text" name="why" placeholder="Why" className="w-full" />
        <Button type="submit" icon="plus" className="w-auto">
          Create
        </Button>
      </form>
    </Dialog>
  )
}
