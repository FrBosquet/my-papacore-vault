...
export const calculateStreak = (notes: MarkdownPage[], referenceDate: DateTime): Streak => {
  if(notes.length === 0) {
  ...
  const dates = [...notes]
    ...
    .filter(note => note.$path.replace('.md', '').replace('Journal/', '').localeCompare(isoReferenceDate) <= 0)
  ...
  if (dates.length === 1) { return { days: 1, type: 'streak' } }
  ...
  for (var i = 1; i < dates.length; i++) {
    ...
    if (diff !== 1) { return { days: i, type: 'streak' } }
  }
