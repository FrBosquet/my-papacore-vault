...
const { projectRoot, configPath } = require('./utils');
...
async function configure() {
  ...
  const expandedPath = finalPath.startsWith('~')
    ? path.join(require('os').homedir(), finalPath.slice(1))
    : finalPath;
  ...
  if (!fs.existsSync(absolutePath)) {
    ...
    console.log('The directory will be created when you run "pnpm run install" or "pnpm run dev".\n');
  }
